...
function getSafeImage(contract: Contract): string {
  if (contract.image && contract.image.startsWith("http")) return contract.image;
  const seeds: Record<string, string> = {
    ...
    "Music": "https://images.unsplash.com/photo-1511192336575-5a79af67a629?auto=format&fit=crop&q=80&w=800",
    "Digital Art": "https://images.unsplash.com/photo-1563089145-599997674d42?auto=format&fit=crop&q=80&w=800",
    ...
    "Literature": "https://images.unsplash.com/photo-1512820790803-83ca734da794?auto=format&fit=crop&q=80&w=800",
    "Design": "https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&q=80&w=800",
    "Performing Arts": "https://images.unsplash.com/photo-1508700115892-45ecd05ae2ad?auto=format&fit=crop&q=80&w=800",
    "Gastronomy": "https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&q=80&w=800",
  };
  return seeds[contract.category] || "https://picsum.photos/seed/lya/800/600";
}
...
function getStatut(units: number) {
...
function PaymentModal({ contract, units, onClose, lang }: PaymentModalProps) {
...
interface DetailModalProps { contract: Contract; units: number; onClose: () => void; onPay: () => void; lang: "FR"|"EN"; }
function DetailModal({ contract, onClose, onPay, units, lang }: DetailModalProps) {
  const T = (fr: string, en: string) => lang === "FR" ? fr : en;
  const totalCost = units * (contract.unitValue || LYA_UNIT_VALUE);
  const revenueRights = ((units * (contract.revenueSharePercentage || 10)) / 10000).toFixed(3);
  const fundingPct = contract.availableUnits
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2">
          ...
          <div className="p-6 border-b lg:border-b-0 lg:border-r border-[#1e2a3a]">
            ...
            <div className="rounded-xl overflow-hidden mb-3" style={{aspectRatio:"16/9"}}>
              <img src={getSafeImage(contract)} alt={contract.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            </div>
            <div className="flex gap-2 mb-5">
              {[0,1,2].map(i => (
                <div key={i} className={`flex-1 rounded-lg overflow-hidden border-2 cursor-pointer transition-opacity ${i===0?"border-[#00d4ff] opacity-100":i===1?"border-transparent opacity-60":"border-transparent opacity-30"}`} style={{aspectRatio:"16/9"}}>
                  <img src={getSafeImage(contract)} alt="" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                </div>
              ))}
            </div>
          ...
          <div className="p-6">
            ...
            <button onClick={onPay} className="w-full bg-[#00ff88] hover:bg-[#00cc66] text-black font-bold font-mono py-4 rounded-xl transition-colors text-sm tracking-widest">
              ✦ {T("SOUTENIR CE PROJET","SUPPORT THIS PROJECT")} — ${totalCost.toFixed(2)}
            </button>
          </div>
        </div>
      </div>
...
interface CardProps { contract: Contract; onDetail: (c: Contract, u: number) => void; lang: "FR"|"EN"; }
function ProjectCard({ contract, onDetail, lang }: CardProps) {
  ...
  return (
    <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-2xl overflow-hidden flex flex-col hover:border-[#1e3a5a] transition-colors">
      {/* Image — hauteur fixe, pas d'étirement */}
      <div
        className="relative cursor-pointer overflow-hidden flex-shrink-0"
        style={{ height: "180px" }}
        onClick={() => onDetail(contract, units)}
      >
        <img
          src={getSafeImage(contract)}
          alt={contract.name}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          ...
          <div className="flex justify-between items-end">
            <h3 className="text-white font-black text-base leading-tight" style={{fontFamily:"Georgia,serif"}}>{contract.name}</h3>
      ...
      <div className="px-4 py-3 space-y-2.5">
        <div className="flex justify-between items-center">
          ...
          <span className="bg-[#1e2a3a] text-[#00d4ff] text-[10px] px-2.5 py-0.5 rounded font-mono">
        ...
        {/* Upgrade Prestige — dropdown jaune, visible uniquement au palier 100 */}
        {statut.isPrestige && (
        ...
        <div className="grid grid-cols-2 gap-2.5">
          ...
          <button
            onClick={() => onDetail(contract, units)}
            className="bg-[#00ff88] text-black py-2.5 rounded-xl font-mono text-[10px] font-bold hover:bg-[#00cc66] transition-colors flex items-center justify-center gap-1 tracking-wider"
          >
            ✦ {T("SOUTENIR","SUPPORT")}
          </button>
        </div>
...
function WhatIsLyaUnit({ lang }: { lang: "FR"|"EN" }) {
  const T = (fr: string, en: string) => lang === "FR" ? fr : en;
  return (
    ...
    <section className="bg-[#080c10] min-h-screen">
      ...
      <div className="px-6 py-10 max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-1 bg-[#0d1117] border border-[#1e2a3a] rounded-lg p-0.5">
            {(["FR","EN"] as const).map(l => (
              <button key={l} onClick={() => setLang(l)} className={`px-3 py-1 rounded text-xs font-mono font-bold transition-all ${lang===l?"bg-[#00d4ff] text-black":"text-gray-400 hover:text-white"}`}>{l}</button>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredContracts.map(contract => (
              <ProjectCard
                key={contract.id}
                contract={contract}
                onDetail={(c, u) => setDetail({ contract: c, units: u })}
                lang={lang}
              />
            ))}
          </div>
