  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isAdminEmail = (e: string) => {
        const admins = [
          'linkyourart@gmail.com', 'lequimejeanbaptiste@gmail.com', 
          'jeanbaptistelequime@gmail.com', 'jean-baptiste.lequime@gmail.com',
          'lyacontactpro@gmail.com', 'linkart@gmail.com',
          'admin@linkyourart.com', 'superadmin@linkyourart.com',
          'linkyourart@ai.studio'
        ];
        return admins.includes(e.toLowerCase().trim());
      };

      const isAdmin = isAdminEmail(email);

      // Auto-heal: check if profile exists, create if not
      try {
      ...
      if (errorMessage.includes('Quota Exceeded') || errorMessage.includes('RESOURCE_EXHAUSTED')) {
        setError(t(
          'Infrastructure limit reached (Daily Quota). Please contact admin or try later.',
          'Limite infrastructure atteinte (Quota Quotidien). Contactez l\'admin ou réessayez plus tard.'
        ));
      ...
      >
        <div className="p-6 md:p-8 relative">
          ...
          <div className="flex flex-col items-center text-center mb-8">
            ...
            <div className="text-[10px] font-bold text-accent-gold uppercase tracking-[0.4em] mt-3 drop-shadow-glow-gold">{t('PROFESSIONAL TERMINAL', 'TERMINAL PROFESSIONNEL')}</div>
          </div>
