...
import { UserProfile, UserRole, getRarityLabel } from '../types';
            ...
            {s.questions.map((q, qi) => (
              <div key={qi} className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(lang === 'FR' ? q.optionsFR : q.optionsEN).map(opt => { const pts = parse(opt); const label = opt.split(' — ')[0]; const sel = answers[step]?.[qi] === pts;
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-8 text-center">
        <div className="max-w-md w-full space-y-6">
          ...
          <h2 className="text-2xl font-black text-white">{T('Espace réservé aux Créateurs', 'Creators-only space')}</h2>
                    ...
                    <div className="flex items-center gap-3 p-4 border-b border-white/6">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-0.5">
                          ...
                          <span className={`text-xs font-black ${proj.rarity==='Signature'?'text-accent-gold':proj.rarity==='Exceptional'?'text-[#a78bfa]':proj.rarity==='Distinguished'?'text-primary-cyan':'text-on-surface-variant/50'}`}>★ {getRarityLabel(proj.rarity, lang)}</span>
                        </div>
