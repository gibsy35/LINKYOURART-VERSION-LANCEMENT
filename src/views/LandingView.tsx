...
export const LandingView: React.FC<LandingViewProps> = ({ onEnterDemo, onViewChange }) => {
  ...
  // ── Registre en direct — vrais projets certifiés (fallback sur le jeu
  // de démonstration si Firestore est vide ou inaccessible)
  const [liveProjects, setLiveProjects] = useState<Contract[]>(() => CONTRACTS.slice(0, 6));
  useEffect(() => {
    const contractsRef = collection(db, 'contracts');
    const q = query(contractsRef, limit(6));
    const unsub = onSnapshot(q,
      (snap) => {
        ...
        // If empty, keep the CONTRACTS fallback already in state.
      },
      (err) => {
        console.warn('[LIVE_REGISTRY] Falling back to demo dataset:', err.code);
      }
    );
    return () => unsub();
  }, []);
        ...
        {stage === 'INTRO' ? (
          ...
          <motion.div key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 2 }} className="relative z-10">
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[80vh] bg-[radial-gradient(ellipse_at_top,rgba(167, 139, 250,0.08)_0%,transparent_70%)]" />
