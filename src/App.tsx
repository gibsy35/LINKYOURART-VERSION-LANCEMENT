...
export default function App() {
  ...
  const [notifications, setNotifications] = useState<{ id: string, title: string, message: string, timestamp: string, read: boolean, type: 'INFO' | 'SUCCESS' | 'WARNING' }[]>([{ id: '1', title: t('SYSTEM INITIALIZED', 'SYSTÈME INITIALISÉ'), message: t('LYA Registry v2.5.0 is now active.', 'Le Registre LYA v2.5.0 est maintenant actif.'), timestamp: new Date().toISOString(), read: false, type: 'INFO' }]);
  ...
  const notify = (msg: string) => { setNotification(msg); setTimeout(() => setNotification(null), 3800); };
        ...
        <main className={`transition-all duration-300 ${(!isAuthView && !isLandingView && currentView !== 'CONTRACT_DETAIL') ? (isSidebarCollapsed ? 'lg:ml-20' : 'lg:ml-72') : ''} ${(!isAuthView && !isLandingView && currentView !== 'CONTRACT_DETAIL') ? 'pt-8 pb-20' : ''} min-h-screen relative flex flex-col shadow-2xl overflow-hidden`}>
          <div className={`max-w-[2000px] mx-auto w-full flex-1 flex flex-col relative ${(!isLandingView && currentView !== 'CONTRACT_DETAIL' && currentView !== 'MECENAT') ? 'px-4 md:px-6' : currentView === 'MECENAT' ? 'px-3 md:px-5' : ''}`}>
            <ErrorBoundary name="View Carrier" resetKey={currentView}>
              ...
              {currentView === 'SIGNUP' && <SignupView onViewChange={handleViewChange} setUser={(u) => { setUser(u); addNotification(t('ACCOUNT CREATED', 'COMPTE CRÉÉ'), t('Your professional account has been successfully initialized.', 'Votre compte professionnel a été initialisé avec succès.'), 'SUCCESS'); }} />}
              {currentView === 'LOGIN' && <LoginView onViewChange={handleViewChange} setUser={(u) => { setUser(u); addNotification(t('LOGIN SUCCESSFUL', 'CONNEXION RÉUSSIE'), t(`Welcome back to the LYA terminal, ${u.displayName}.`, `Bon retour sur le terminal LYA, ${u.displayName}.`), 'SUCCESS'); }} onGuestBrowse={() => {
                try { sessionStorage.setItem('lya_visitor_mode', 'true'); } catch { /* noop */ }
                setShowPublicHome(false);
                setCurrentView('MECENAT');
              }} />}
              ...
              {currentView === 'CREATOR_DASHBOARD' && <CreatorDashboardView user={effectiveUser} onNotify={notify} onViewChange={handleViewChange} onSelectContract={(c) => { setViewingContract(c); setCurrentView('CONTRACT_DETAIL'); }} />}
              {currentView === 'PATRON_DASHBOARD' && <InvestorDashboardView user={effectiveUser} onNotify={notify} onViewChange={handleViewChange} />}
              {currentView === 'PROFESSIONAL_DASHBOARD' && <ProfessionalDashboardView user={effectiveUser} onNotify={notify} onViewChange={handleViewChange} onSelectContract={(c) => { setViewingContract(c); setCurrentView('CONTRACT_DETAIL'); }} />}
              ...
              {!['HOME','LANDING','DASHBOARD','VALIDATION','REGISTRY','LINK_ART','LOUNGE','WALLET','SIGNUP','LOGIN','RESET_PASSWORD','PROFILE','PRICING','SWIPE','MECENAT','BROCHURE','WATCHLIST','SETTINGS','COMPARE','SOCIAL_FEED','PAYMENT','CONTRACT_DETAIL','TERMS','PRIVACY','LEGAL_REGISTRY','GOVERNANCE','API','ACADEMY','APPLY_VERIFICATION','ABOUT','ADMIN_PANEL','ISSUER_PROFILE','OUR_MODEL','FAQ','LEGAL_MENTIONS','CREATOR_DASHBOARD','PATRON_DASHBOARD','PROFESSIONAL_DASHBOARD','PROJECT_PUBLIC','CREATOR_PROFILE'].includes(currentView) && (<NotFoundView onViewChange={handleViewChange} />)}
              {currentView === 'CONTRACT_DETAIL' && viewingContract && (<ContractDetailView contract={liveContracts.find(c => c.id === viewingContract.id) || viewingContract} onBack={() => setCurrentView('REGISTRY')} onNotify={notify} isWatchlisted={watchlist.includes(viewingContract.id)} onToggleWatchlist={handleToggleWatchlist} user={effectiveUser} />)}
