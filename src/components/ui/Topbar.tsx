...
export const Topbar: React.FC<TopbarProps> = ({
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-14 z-[90] transition-all duration-300">
      ...
      <div className="absolute inset-0 bg-surface-dim/95 backdrop-blur-xl border-b border-chrome -z-10" />
      ...
      <div className={`h-full flex items-center justify-between relative px-6 md:px-12 transition-all duration-300 ${isSidebarCollapsed ? 'lg:pl-24' : 'lg:pl-72'}`}>
        ...
        <div className="flex items-center gap-4 lg:hidden">
          <button onClick={onToggleMobileMenu} className="text-on-surface-variant hover:text-on-surface transition-colors">
            <Menu size={20} />
          </button>
          <div className="flex items-center gap-3">
            <Logo size={24} color="multi" showBeta />
            <span className="text-on-surface font-black tracking-tighter text-sm hidden sm:block">LINKYOURART</span>
          </div>
        </div>

        {/* Desktop Ticker Wrapper - Extended to the left */}
        <div className="hidden lg:flex flex-1 h-full items-center overflow-hidden bg-surface-high/50 relative">
          <motion.div 
            ...
            {(() => {
              ...
              [...CONTRACTS].slice(0, 14).forEach((item, i) => {
                ...
                rows.push({
                  key: `score-${item.id}-${i}`,
                  node: (
                    <div className="flex items-center gap-6 cursor-pointer group h-full px-4 border-l-2 border-primary-cyan/30 hover:bg-overlay-soft transition-colors"
                      onClick={() => onSelectContract ? onSelectContract(item) : onViewChange('DASHBOARD')}>
                      <span className="text-[10px] font-black text-on-surface group-hover:text-primary-cyan transition-colors uppercase tracking-widest flex items-center gap-2">
                        <span className="text-on-surface-variant/40">{item.registryIndex}</span>
                        {item.name}
                      </span>
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] text-on-surface font-mono font-bold">{item.totalScore}/1000</span>
                  ...
                  rows.push({
                    ...
                    node: (
                      <div className="flex items-center gap-2 h-full px-4 cursor-pointer group border-l-2 border-accent-gold/30 hover:bg-accent-gold/5 transition-colors"
                        ...
                        <span className="text-[10px] font-black text-accent-gold uppercase tracking-widest group-hover:text-on-surface transition-colors">{item.name}:</span>
        ...
        <div className="flex items-center gap-2 md:gap-4 lg:gap-6">
          ...
          <button
            ...
            className="w-10 h-10 flex items-center justify-center bg-overlay-soft hover:bg-overlay-heavy border border-chrome rounded-full text-on-surface-variant hover:text-primary-cyan transition-all group relative"
          ...
          <div className="hidden lg:flex items-center gap-1 bg-overlay-soft p-0.5 rounded-full border border-chrome">
            {[
              { id: 'EUR', symbol: '€' },
              { id: 'USD', symbol: '$' },
              { id: 'GBP', symbol: '£' },
              ...
              <button
                ...
                className={`w-7 h-7 flex items-center justify-center text-[10px] font-black transition-all rounded-full ${
                  currency === curr.id 
                    ...
                    : 'text-on-surface-variant/60 hover:text-on-surface hover:bg-overlay-heavy'
                }`}
                title={curr.id}
              >
                {curr.symbol}
              ...
              {isNotifMenuOpen && (
                <>
                  ...
                  <motion.div 
                    ...
                    className="absolute top-full right-0 mt-4 w-80 bg-surface-dim border border-chrome shadow-2xl z-[101] font-mono"
                  >
                    <div className="p-4 border-b border-chrome flex items-center justify-between">
                      <span className="text-[10px] font-black text-on-surface uppercase tracking-widest">{t('SYSTEM LOGS', 'LOGS SYSTÈME')}</span>
                    ...
                    <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map(notif => {
                          ...
                          }[notif.type as 'SUCCESS' | 'WARNING' | 'ERROR' | 'INFO'] || 'bg-overlay-soft border-chrome text-on-surface';

                          return (
                            <div 
                              key={notif.id} 
                              className={`p-4 border-b border-chrome hover:bg-overlay-heavy transition-all cursor-pointer relative group/notif ${!notif.read ? 'bg-primary-cyan/5' : ''}`}
                            >
          ...
          <div className="relative">
            {user ? (
              ...
              >
                <div className="w-9 h-9 rounded-full bg-overlay-soft border border-chrome flex items-center justify-center group-hover:border-primary-cyan transition-all overflow-hidden relative">
                  {user.avatarUrl ? (
                    <img 
                ...
                <div className="hidden md:block text-left">
                  <div className="text-[10px] font-black text-on-surface uppercase tracking-tight group-hover:text-primary-cyan transition-colors">{user.displayName}</div>
              ...
              <button 
                ...
                className="px-6 py-1.5 bg-primary-cyan text-surface-dim text-[10px] font-black uppercase tracking-widest hover:opacity-85 transition-all transform hover:-translate-y-0.5 active:translate-y-0 rounded-full"
              >
                {t('SIGN IN', 'CONNEXION')}
              </button>
            )}
              ...
              {isUserMenuOpen && user && (
                <>
                  ...
                  <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    className="absolute top-full right-0 mt-4 w-56 bg-surface-dim border border-chrome shadow-2xl z-[101] overflow-hidden font-mono"
                  >
                    <div className="p-4 border-b border-chrome bg-overlay-soft">
                      <div className="text-[10px] font-black text-on-surface uppercase mb-1">{user.displayName}</div>
                    ...
                    <div className="p-2">
                      {user.role === UserRole.ADMIN && (
                        <button 
                          ...
                          className="w-full flex items-center gap-3 px-3 py-2 text-[10px] font-bold text-accent-gold hover:text-on-surface hover:bg-accent-gold/10 transition-all uppercase tracking-widest border border-accent-gold/20 mb-1 rounded-sm"
                        >
                      ...
                      <button 
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2 text-[10px] font-bold text-on-surface-variant hover:text-on-surface hover:bg-overlay-soft transition-all uppercase tracking-widest"
                      >
                        <User size={14} /> {
                      ...
                      <div className="h-px bg-overlay-soft my-2" />
                      <button 
