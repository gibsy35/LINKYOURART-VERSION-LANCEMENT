...
export const ContractCard: React.FC<ContractCardProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <div className={`relative overflow-hidden ${compact ? 'hidden' : 'aspect-video rounded-t-[2rem]'}`}>
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2 z-30">
          <motion.div 
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-primary-cyan text-surface-dim text-[9px] font-black uppercase tracking-widest rounded-sm shadow-[0_0_20px_rgba(0,224,255,0.4)] border border-white/20"
          >
            <Coins size={10} className="animate-bounce" />
            LYA UNIT $50
          </motion.div>
      ...
      {/* Content */}
      <div className={`flex-1 ${compact ? '' : 'p-4 flex flex-col justify-between'}`}>
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="flex gap-2">
              <span className="text-[7px] font-black uppercase tracking-[0.3em] text-primary-cyan bg-primary-cyan/5 px-2 py-1 border border-primary-cyan/10">
                {contract.registryIndex}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[7px] font-black uppercase tracking-widest text-on-surface-variant/40">{t('CREATIVE_CONTRACT', 'CONTRAT CRÉATIF')}</span>
            </div>
          </div>
          
          <h3 className="text-base font-black font-headline tracking-tighter uppercase text-white group-hover:text-primary-cyan transition-colors mb-2 leading-tight italic">
            {contract.name}
          </h3>

          {/* Scores - Always Visible */}
          <div className="flex gap-2 mb-3">
            <div className="flex-1 flex flex-col items-center justify-center py-1.5 bg-surface-dim/40 border border-white/5 rounded-lg border-b-2 border-b-white/10">
              <div className="text-[6px] font-black uppercase tracking-widest text-on-surface-variant/60 leading-none mb-1">ALGO</div>
              <div className="text-[11px] font-black font-headline tracking-tighter text-white italic">
                {scoreAlgoValue}
              </div>
            </div>
            <div className="flex-1 flex flex-col items-center justify-center py-1.5 bg-surface-dim/40 border border-white/5 rounded-lg border-b-2 border-b-white/10">
              <div className="text-[6px] font-black uppercase tracking-widest text-on-surface-variant/60 leading-none mb-1">PRO</div>
              <div className="text-[11px] font-black font-headline tracking-tighter text-white italic">
                {scoreProValue}
              </div>
            </div>
            <div className="flex-1 flex flex-col items-center justify-center py-1.5 bg-primary-cyan/10 border border-primary-cyan/20 rounded-lg relative overflow-hidden group/score border-b-2 border-b-primary-cyan/30">
              <div className="absolute inset-0 bg-primary-cyan/5 opacity-0 group-hover/score:opacity-100 transition-opacity" />
              <div className="text-[6px] font-black uppercase tracking-widest text-primary-cyan leading-none mb-1">FINAL</div>
              <div className="text-[11px] font-black font-headline tracking-tighter text-primary-cyan italic">
                {scoreFinalValue}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[8px] font-bold text-on-surface-variant uppercase tracking-widest leading-none opacity-60 font-mono">{contract.jurisdiction} PROTECTED</span>
          </div>

          <div className="grid grid-cols-2 gap-4 pb-3 mb-2 border-b border-white/5">
            <div>
              <div className="text-[10px] text-on-surface-variant/50 uppercase tracking-widest font-black mb-1">{t('MINT_PRICE', 'PRIX DE MINT')}</div>
              <div className="text-sm font-black text-white font-headline tracking-tight">{formatPrice(contract.unitValue)}</div>
              <div className="text-[7px] text-accent-gold font-black tracking-widest mt-1 uppercase italic">GUARANTEED CREATIVE VALUE</div>
            </div>
            <div className="text-right">
              <div className="text-[10px] text-on-surface-variant/50 uppercase tracking-widest font-black mb-1 font-headline">{t('GROWTH_CAP', 'CAP DE CROISSANCE')}</div>
        ...
        <div className="flex items-center justify-between mt-auto">
          <div className="flex flex-col gap-1.5 px-3 py-2 bg-white/5 rounded-sm border border-white/5 group-hover:bg-white/10 transition-all">
            <span className="text-[7px] font-black text-on-surface-variant uppercase tracking-[0.1em] opacity-40">{t('CREATIVE_LICENSE', 'LICENCE CRÉATIVE')}</span>
            <span className="text-[10px] font-black text-white uppercase italic tracking-tighter">UNLIMITED IP ACCESS</span>
          </div>
          
          <div className="flex flex-col items-end gap-1.5">
            <span className="text-[7px] font-black text-primary-cyan uppercase tracking-[0.2em]">{t('COMMUNITY_HOLDERS', 'DÉTENTEURS COMMUNAUTÉ')}</span>
            <div className="flex -space-x-2">
              {[1, 2, 3].map(i => (
                <div key={i} className="w-7 h-7 rounded-full border-2 border-surface-low bg-white/10 flex items-center justify-center text-[8px] font-black overflow-hidden relative grayscale group-hover:grayscale-0 transition-all shadow-lg">
                  <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${contract.id}${i}`} alt="user" className="w-full h-full object-cover" />
                </div>
              ))}
              <div className="w-7 h-7 rounded-full border-2 border-surface-low bg-surface-high flex items-center justify-center text-[7px] font-black text-white relative z-10 shadow-lg">
                +84
              </div>
            </div>
          </div>
