...
export const PressMediaSection: React.FC<Props> = ({ t, language, onNotify }) => {
  ...
  const handleAccred = async () => {
    ...
    try {
      ...
      onNotify(t('✦ LYA Press accreditation request sent — 5 business days.', '✦ Demande d\'accréditation LYA Press envoyée — 5 jours ouvrés.'));
    } catch(e) {
      onNotify(t('Network error', 'Erreur réseau'));
    }
  };
        ...
        <div className="relative z-10 grid md:grid-cols-2 gap-8 items-center">
          <div className="space-y-4">
            ...
            <h2 className="text-2xl font-black text-white leading-tight">
              ...
              <span className="text-[#a78bfa]">{t('The work before the author.', 'L\'œuvre avant l\'auteur.')}</span>
            </h2>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="px-2 py-0.5 bg-emerald-400/10 border border-emerald-400/20 rounded-full text-[9px] font-black text-emerald-400">● {CONTRIBUTIONS.length} {t('open', 'ouverts')}</span>
          </div>
          {/* Filtres */}
        ...
        <div className="relative z-10 grid md:grid-cols-3 gap-6 items-center">
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1">
              {[
                ...
                t('Signature under LYA pseudonym', 'Signature sous pseudonyme LYA'),
              ].map((item, i) => (
          ...
          <div className="space-y-3">
            <button onClick={() => setAccredModal(true)}
              ...
              {accredSubmitted ? t('✓ Request sent', '✓ Demande envoyée') : t('Request accreditation', 'Demander l\'accréditation')}
            </button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-[9px] font-black text-white/40 uppercase tracking-widest block mb-1">{t('Your approach *', 'Votre approche *')}</label>
        ...
        {accredModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-[9px] font-black text-white/40 uppercase tracking-widest block mb-1">{t('Profile type *', 'Type de profil *')}</label>
                  <select value={accredForm.type} onChange={e => setAccredForm(f => ({...f, type: e.target.value}))}
                    ...
                    <option value="journalist">{t('Journalist', 'Journaliste')}</option>
                ...
                <div>
                  ...
                  <input value={accredForm.experience} onChange={e => setAccredForm(f => ({...f, experience: e.target.value}))}
                    placeholder={t('Sectors covered, years of experience...', 'Secteurs couverts, années d\'expérience...')}
                    className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 outline-none focus:border-accent-gold/50"/>
                </div>
                <div>
                  ...
                  <textarea value={accredForm.message} onChange={e => setAccredForm(f => ({...f, message: e.target.value}))}
                    rows={3} placeholder={t('Why do you want to join LYA Press?', 'Pourquoi souhaitez-vous rejoindre LYA Press ?')}
