...
export const ContractDetailView: React.FC<ContractDetailViewProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'financials' | 'legal' | 'milestones' | 'messaging' | 'visualization'>('overview');
  ...
  const handleGenerateAiAnalysis = async () => {
    ...
    try {
      ...
      const analysis = await generateAssetAnalysis(contract.name, contract.description, contract.totalScore || 0);
      setAiAnalysis(analysis);
    } catch (error) {
      console.error('Analysis failed', error);
      onNotify(t('ANALYSIS SYNTHESIS FAILED.', 'ÉCHEC DE LA SYNTHÈSE D\'ANALYSE'));
    } finally {
  ...
  const handleGenerateVisualization = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        contents: `Generate a highly professional, cinematic, and detailed concept art for a creative rights contract titled "${contract.name}" in the category of ${contract.category}. The visual should represent ${contract.description.substring(0, 200)}. Use a premium, modern, and high-tech aesthetic suitable for an institutional investment platform.`,
      });
      ...
      for (const part of response.candidates[0].content.parts) {
        if (part.inlineData) {
          ...
          setVisualUrl(url);
          onNotify(t('AI VISUALIZATION GENERATED SUCCESSFULLY.', 'VISUALISATION IA GÉNÉRÉE AVEC SUCCÈS.'));
          break;
        }
      }
    } catch (error) {
      console.error('Visualization failed', error);
      onNotify(t('VISUALIZATION GENERATION FAILED.', 'ÉCHEC DE LA GÉNÉRATION DE VISUALISATION.'));
    } finally {
        ...
        <div className="lg:col-span-2 space-y-8">
            ...
            <div className="relative h-full flex flex-col justify-end p-6 md:p-8">
              <div className="flex flex-wrap lg:flex-nowrap items-end justify-between gap-6">
                <div className="space-y-4">
                  ...
                  <p className="text-xs md:text-sm text-white/80 max-w-xl leading-relaxed text-justify">
          ...
          <div className="flex border-b border-white/10 overflow-x-auto no-scrollbar whitespace-nowrap scroll-smooth pb-1 -mx-6 px-6 md:mx-0 md:px-0">
            <div className="flex min-w-max">
              {[
                ...
                { id: 'visualization', label: t('Visualization', 'Visualisation'), icon: <Zap size={16} /> },
          ...
          <div className="min-h-[400px]">
                ...
                <div className="col-span-1 md:col-span-2 bg-primary-cyan/5 border border-primary-cyan/20 rounded-[2.5rem] p-10 space-y-8 relative overflow-hidden group">
                  ...
                  {aiAnalysis ? (
                    ...
                    >
                      ...
                      <p className="text-lg md:text-xl font-light text-white leading-relaxed italic opacity-90 indent-8 text-justify">
                        "{aiAnalysis}"
                      </p>
                    ...
                    <div className="py-16 border-2 border-dashed border-white/5 rounded-3xl flex flex-col items-center justify-center text-center space-y-6 group-hover:border-primary-cyan/20 transition-all">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-[10px] text-on-surface-variant/40 uppercase tracking-widest leading-relaxed max-w-sm text-justify">
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-xl space-y-2">
                      ...
                      <p className="text-[11px] text-on-surface/80 leading-relaxed italic line-clamp-4 text-justify">{investmentThesis.bullCase}</p>
                    </div>
                    <div className="p-4 bg-accent-pink/5 border border-accent-pink/20 rounded-xl space-y-2">
                      ...
                      <p className="text-[11px] text-on-surface/80 leading-relaxed italic line-clamp-4 text-justify">{investmentThesis.bearCase}</p>
                    </div>
                  </div>
            ...
            <div className="bg-surface-low/30 backdrop-blur-2xl border border-white/10 rounded-2xl p-8 space-y-6">
              ...
              <p className="text-[10px] text-on-surface-variant uppercase tracking-widest leading-relaxed text-justify">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  {[
                    ...
                    <div key={i} className="bg-surface-low/40 backdrop-blur-xl border border-white/10 p-6 rounded-2xl group hover:border-primary-cyan/20 transition-all">
                      ...
                      <p className="text-[10px] text-on-surface-variant/60 leading-relaxed uppercase font-bold tracking-widest text-justify">{risk.desc}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="bg-surface-dim border border-primary-cyan/20 rounded-2xl overflow-hidden">
                  <div className="bg-primary-cyan/10 p-6 border-b border-primary-cyan/20 flex justify-between items-center">
                    <div className="space-y-1">
                      ...
                      <p className="text-[8px] text-primary-cyan font-black uppercase tracking-widest opacity-40 text-justify">{t('Verified registry for creative rights transfer', 'Registre vérifié pour le transfert de droits créatifs')}</p>
                    </div>
                ...
                <div className="bg-surface-low/30 backdrop-blur-2xl border border-white/10 rounded-2xl p-8 space-y-8">
                  <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                    <div className="space-y-2">
                      <h3 className="text-xs font-black uppercase tracking-[0.3em] text-primary-cyan flex items-center gap-2">
                        <Zap size={14} />
                        {t('Dynamic Asset Visualization', 'Visualisation Dynamique de l\'Actif')}
                      </h3>
                      <p className="text-[10px] text-on-surface-variant uppercase tracking-widest leading-relaxed opacity-60 text-justify">
                        {t('Generative AI representation based on underlying creative rights and market positioning.', 'Représentation par IA générative basée sur les droits créatifs sous-jacents et le positionnement du marché.')}
                      </p>
                    </div>
                    ...
                    <button 
                      ...
                      className="px-10 py-4 bg-accent-gold text-surface-dim text-[11px] font-black uppercase tracking-[0.4em] rounded-2xl hover:bg-white hover:scale-105 transition-all shadow-[0_0_30px_rgba(245,158,11,0.3)] disabled:opacity-50 flex items-center gap-3"
                    >
                      {isVisualizing ? (
                        <>
                          <div className="w-4 h-4 border-2 border-surface-dim border-t-transparent rounded-full animate-spin" />
                        ...
                        <>
                          <Sparkles size={16} />
                          {t('GENERATE VISUALIZATION', 'GÉNÉRER LA VISUALISATION')}
                        </>
                      )}
                    </button>
                  </div>
                    ...
                    {visualUrl ? (
                      <>
                        ...
                        <div className="absolute bottom-8 left-8 right-8">
                           <div className="flex items-center gap-3 mb-2">
                             <div className="px-3 py-1 bg-primary-cyan/20 backdrop-blur-md border border-primary-cyan/30 rounded-sm text-[8px] font-black text-primary-cyan uppercase tracking-widest">
                               {t('AI GENERATED_PROTOTYPE', 'PROTOTYPE_GÉNÉRÉ_PAR_IA')}
                             </div>
                             <div className="text-[8px] font-bold text-white/40 uppercase tracking-widest">
                               MDL: GEMINI-2.5-PRO-VISION
                             </div>
                           </div>
                           <p className="text-sm font-bold text-white italic opacity-80 line-clamp-2">
                             {contract.description}
                           </p>
                        </div>
                      </>
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center space-y-6">
                        <div className="w-24 h-24 bg-white/5 rounded-full flex items-center justify-center text-on-surface-variant/20 animate-pulse border border-white/5">
                           <Zap size={48} />
                        </div>
                        <div className="text-center space-y-2">
                           <p className="text-xs text-on-surface-variant uppercase tracking-[0.3em] font-black">{t('NO VISUALIZATION DATA', 'AUCUNE DONNÉE DE VISUALISATION')}</p>
                           <p className="text-[10px] text-on-surface-variant/40 uppercase tracking-widest leading-relaxed max-w-sm px-6 text-justify">
                             {t('Click above to initialize the Gemini-powered visualization engine for this creative rights project.', 'Cliquez ci-dessus pour initialiser le moteur de visualisation alimenté par Gemini pour ce projet de droits créatifs.')}
                           </p>
                        </div>
                      </div>
                    )}
                ...
                <div className="bg-primary-cyan/5 border border-primary-cyan/20 rounded-2xl p-8 space-y-6 relative overflow-hidden">
                   ...
                   {aiAnalysis ? (
                     ...
                     <div className="py-12 border-2 border-dashed border-white/5 rounded-xl flex flex-col items-center justify-center text-center space-y-4">
                        ...
                        <p className="text-[10px] text-on-surface-variant uppercase tracking-widest max-w-xs text-justify">{t('Click synthesize to generate an exhaustive professional overview of this creative contract using LYA Intelligence Grid.', 'Cliquez sur synthétiser pour générer un aperçu professionnel exhaustif de ce contrat créatif via la Grille d\'Intelligence LYA.')}</p>
                     </div>
                   )}
                </div>
