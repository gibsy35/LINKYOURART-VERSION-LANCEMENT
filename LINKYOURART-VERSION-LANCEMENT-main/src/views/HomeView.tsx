    ...
    <section className="relative z-10 py-32 px-6 bg-surface-low/20">
      <div className="max-w-[1800px] mx-auto mt-16">
        <div className="text-center mb-16 px-4">
          ...
          <p className="text-on-surface text-lg max-w-3xl mx-auto leading-relaxed mb-4 text-justify">
          ...
          <p className="text-[11px] font-mono text-primary-cyan uppercase tracking-wider max-w-2xl mx-auto opacity-80 border border-primary-cyan/20 bg-primary-cyan/5 p-4 rounded-sm text-justify">
          ...
          >
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center gap-6 sm:gap-8 mb-10 sm:mb-12 lg:mb-16">
              <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 text-center sm:text-left">
                ...
                <div>
                  <h3 className="text-xl sm:text-3xl font-black text-white uppercase leading-tight">{categories.find(c => c.id === selectedCategory)?.label}</h3>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between gap-6 sm:gap-8">
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-3 mb-2">
                          <h4 className="text-lg sm:text-xl font-black text-white uppercase">{m.title}</h4>
                        ...
                        <p className="text-xs sm:text-sm text-on-surface leading-relaxed opacity-60 text-justify">{m.desc}</p>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 shrink-0 items-stretch sm:items-center">
                        ...
                        <div className="px-4 py-2 bg-primary-cyan/5 border border-primary-cyan/10 rounded-sm min-w-[140px] flex-1 sm:flex-none">
                          ...
                          <div className={`text-[9px] font-black mt-1 text-right ${m.type === 'positive' ? 'text-emerald-400' : 'text-red-400'}`}>
                            {m.variation}
                          </div>
                        </div>
                      </div>
            ...
            <div className="mt-16 text-center pt-8 border-t border-white/5">
              ...
              <p className="text-sm text-on-surface-variant opacity-60 max-w-2xl mx-auto leading-relaxed mb-8 text-justify">
...
const ProjectCard: React.FC<ProjectCardProps & { score?: string; status?: string; color?: string; isAbsoluteHighest?: boolean; isHighTier?: boolean }> = ({ label, sub, budget, icon, active, onClick, score = '742', status = 'VALIDATED', color = 'primary-cyan', isAbsoluteHighest = false, isHighTier = false }) => {
  const { t } = useTranslation();
  const { formatPrice } = useCurrency();
  
  // Adjusted beam values based on tiers
  const beamOpacity = isAbsoluteHighest ? 'opacity-100' : isHighTier ? 'opacity-40' : active ? 'opacity-20' : 'group-hover:opacity-30';
  const beamBlur = isAbsoluteHighest ? 'blur-[80px]' : isHighTier ? 'blur-3xl' : 'blur-3xl';
  const beamGlow = isAbsoluteHighest ? '100' : isHighTier ? '40' : '20';
  const pulseOpacity = isAbsoluteHighest ? [0.4, 0.8, 0.4] : isHighTier ? [0.2, 0.4, 0.2] : [0.1, 0.2, 0.1];
  ...
  return (
    <motion.div 
      ...
      className={`p-4 sm:p-6 border transition-all text-left relative group min-w-[220px] sm:min-w-[280px] h-[320px] sm:h-[380px] cursor-pointer flex flex-col justify-between ${
        isAbsoluteHighest 
          ? `bg-primary-cyan/10 border-primary-cyan shadow-[0_0_80px_rgba(0,224,255,0.6)]` 
          : isHighTier
            ? `bg-primary-cyan/5 border-primary-cyan/30 shadow-[0_0_40px_rgba(0,224,255,0.2)]`
            : active
              ? `bg-surface-low border-white/10 shadow-[0_0_15px_rgba(255,255,255,0.05)]`
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden rounded-[inherit] pointer-events-none z-0">
        <motion.div 
          initial={{ rotate: 0 }}
          animate={{ rotate: 360 }}
          transition={{ duration: isAbsoluteHighest ? 2 : isHighTier ? 6 : 12, repeat: Infinity, ease: "linear" }}
          className={`absolute -inset-[150%] transition-opacity duration-700 ${beamOpacity}`}
        >
          <div className={`w-full h-full bg-conic-gradient from-transparent via-primary-cyan/${beamGlow} to-transparent ${beamBlur}`} />
        </motion.div>
        ...
        <div className={`absolute inset-0 border-[2px] transition-all duration-700 ${
          isAbsoluteHighest 
            ...
            : isHighTier
              ? 'border-primary-cyan/50 opacity-80 shadow-[0_0_15px_rgba(0,224,255,0.3)]'
              : active 
                ? 'border-white/20 opacity-60 shadow-[0_0_10px_rgba(255,255,255,0.1)]' 
        ...
        {(active || isAbsoluteHighest || isHighTier) && (
          <motion.div 
            animate={{ 
              opacity: pulseOpacity,
              scale: isAbsoluteHighest ? [1, 1.1, 1] : isHighTier ? [1, 1.05, 1] : [1, 1.01, 1]
            }}
            transition={{ duration: isAbsoluteHighest ? 2 : isHighTier ? 3 : 4, repeat: Infinity, ease: "easeInOut" }}
            className={`absolute inset-0 bg-primary-cyan/${isAbsoluteHighest ? '40' : isHighTier ? '20' : '10'} blur-3xl z-[-1]`}
          />
        )}
      </div>
      ...
      <div className="absolute top-4 right-4 z-20">
        <div className="flex flex-col items-center bg-black/40 backdrop-blur-md p-2 rounded-lg border border-white/5">
          ...
          <div className={`text-base font-black tracking-tighter ${active ? 'text-primary-cyan' : 'text-white'}`}>{score}</div>
        </div>
      </div>
      ...
      <div className="relative z-10 mt-12 px-1">
        ...
        <div className="text-2xl font-black text-white uppercase mb-1 tracking-tighter leading-none">{label}</div>
        ...
        <div className="space-y-1.5 mb-6">
          <div className="flex justify-between text-[8px] font-black uppercase tracking-widest opacity-60">
            ...
            <span className="text-primary-cyan">68% / {formatPrice(250000)}</span>
          </div>
      ...
      <div className="relative z-10">
        ...
        <div className="flex justify-between items-end">
          <div className="flex flex-col gap-1">
            ...
            <div className="text-sm font-black text-white group-hover:text-primary-cyan transition-colors">
              {typeof budget === 'number' ? formatPrice(budget) : budget}
            </div>
          </div>
...
const ProjectSlider = ({ onNav }: { onNav: (v: View) => void }) => {
  ...
  const { formatPrice } = useCurrency();
  const projects = [
    { id: 'film', label: t('Feature Film', 'Long-métrage'), sub: t('Professional Cinema Rights', 'Droits Cinéma Professionnels'), budget: 1500000, score: '978', status: 'AUDITED', color: 'primary-cyan', icon: <Clapperboard /> },
    { id: 'music', label: t('Music Album', 'Album Musical'), sub: t('Verified Creative Portfolio', 'Portfolio Créatif Vérifié'), budget: 150000, score: '984', status: 'VALIDATED', color: 'rose-400', icon: <Music /> },
    { id: 'series', label: t('TV Series', 'Série TV'), sub: t('Digital Content Rights', 'Droits de Contenu Digital'), budget: 400000, score: '992', status: 'MINTING', color: 'indigo-400', icon: <Video /> },
    { id: 'comedy', label: t('Musical Comedy', 'Comédie Musicale'), sub: t('Creative Arts Index', 'Index des Arts Créatifs'), budget: 250000, score: '965', status: 'PENDING', color: 'accent-gold', icon: <Zap /> },
  ];
    ...
    <div id="discover-projects" className="relative w-full overflow-hidden py-10 mt-12 lg:mt-0 flex items-center min-h-[400px] sm:min-h-[450px]">
      <div className="flex gap-4 sm:gap-6 animate-marquee">
        {[...projects, ...projects, ...projects].map((p, i) => {
          // Absolute highest is Series (992)
          const isHighest = p.score === '992'; 
          // Film (978) gets a second-tier beam but less intense
          const isHigh = p.score === '978' || p.score === '984'; 
...
export const HomeView: React.FC<HomeViewProps> = ({ user, onViewChange }) => {
  ...
  return (
    <div className="relative min-h-screen bg-surface-dim overflow-x-hidden">
      ...
      <section id="project-section" className="relative z-10 pt-4 pb-12 lg:pb-20 px-6 lg:px-12 overflow-hidden flex items-center min-h-[max(600px,85vh)]">
        ...
        <div className="max-w-[1800px] mx-auto">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter mb-8 uppercase leading-none">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6 relative">
              ...
              {[
                { 
                  ...
                  desc: t('Contractual rights are immutable and audited by authorized legal nodes.', 'Les droits contractuels sont immuables et audités par des nœuds légaux autorisés.'),
                  icon: <Shield size={24} />
                },
                { 
                  id: '02', 
                ...
                { 
                  ...
                  desc: t('Institutional grade compliance with SEC, MiCA & FCA standards.', 'Conformité de grade institutionnel aux normes SEC, MiCA & FCA.'),
                  icon: <Scale size={24} />
                }
              ].map((item, i) => (
                <motion.div
                ...
                >
                  <div className="absolute top-0 right-0 p-4 opacity-[0.03] group-hover:opacity-[0.1] transition-opacity">
                    <span className="text-6xl font-black italic">{item.id}</span>
                  </div>
                  ...
                  <h3 className="text-lg font-black text-white uppercase italic mb-3 group-hover:text-primary-cyan transition-colors">{item.title}</h3>
